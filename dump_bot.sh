...
if [ $t_back -ge $c_back ]
	then
		...
		curl -X POST -d text="Бэкап завершен $date %0AКоличество доступных копий: $c_back %0AРазмер последней копии: $b Mb " 'https://api.telegram.org/bot5892481615:AAH01YAOHd39_LU3HeJsfGU-YF7GC34a4do/sendMessage?chat_id=-1001884359879' 
	else 
		pg_dump -U postgres -d back_test > /test/back_test-$date
fi
